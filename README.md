...
1. conda create -n borges_env_repr python=3.10.9 -y
2. grep "pypi_0" borges_req.txt | sed 's/=pypi_0//' | sed 's/=/==/' | grep -v mediapipe > borges_cuda_requirements.txt
3. pip install mediapipe==0.10.21
4. pip install -r borges_cuda_requirements.txt
5. pip install tiktoken==0.5.2
